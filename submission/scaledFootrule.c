...
static int * assign(int size, double matrix[size][size], int * currentAssignments)
{
	...
	for(i = 0; i < size; i++) {
		...
		if(zeroCount == 1) {
			zeroCount = 0;
			for(k = 0; k < size; k++) {
				if(assignments[k] == columnAssigned) {
					zeroCount++;
					break;
				}
			}
			if(!zeroCount) assignments[i] = columnAssigned;
		}
	}
...
static int reduceMatrix(int size, double matrix[size][size])
{
	...
	int assigned[size][size];
	
	for(i = 0; i < size; i++) row[i] = 1;
    memset(column, 0, sizeof(column));
	memset(assigned, 0, sizeof(assigned));
	
	/* Step 1: Assign as many zeros as possible. */
	
	int * assignments = malloc(sizeof(int) * size);
    memset(assignments, -1, sizeof(int) * size);
	assignments = assign(size, matrix, assignments);
	
	for(i = 0; i < size; i++) {
		if(assignments[i] != -1) {
			row[i] = 0;
			assigned[i][assignments[i]] = 1;
			continue;
		}
		assignedColumn = -1;
		for(j = 0; j < size; j++) {
			if(!matrix[i][j]) {
				...
				else assignedColumn = -1;
			}
		}
		if(assignedColumn != -1) {
			assigned[i][assignedColumn] = 1;
			row[i] = 0;
		}
	}
	...
	/* Step 3: Subtract minimum from uncovered and add to intersection
	of double covered. */
	...
	for(i = 0; i < size; i ++) {
		...
		for(j = 0; j < size; j++) {
			...
			if(matrix[i][j] < minimum || minimum == -1) {
				minimum = matrix[i][j];
			}
		}
	}
	...
	for(i = 0; i < size; i ++) {
		for(j = 0; j < size; j++) {
			if(row[i] && !column[j]) matrix[i][j] -= minimum;
			else if(!row[i] && column[j]) matrix[i][j] += minimum;
		}
	}
