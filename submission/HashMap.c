...
{
	void ** keys = malloc(sizeof(void *) * sizeHashMap(map));
	Queue queue;
	HashEntry entry;
	...
	for(i = 0; i < map->capacity; i++) {
		if(map->buckets[i]) {
			queue = getQueueBST(map->buckets[i]);
			while(!emptyQueue(queue)) {
				entry = nextQueue(queue);
				keys[count++] = entry->key;
			}
		}
	}
	
	if(!map->resizing) return keys;
	
	for(i = 0; i < map->old->capacity; i++) {
		if(map->old->buckets[i]) {
			queue = getQueueBST(map->old->buckets[i]);
			while(!emptyQueue(queue)) {
				entry = nextQueue(queue);
				keys[count++] = entry->key;
			}
