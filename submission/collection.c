...
HashMap getUrlCount(PriorityQueue terms)
{
	HashMap words = newStringHashMap();
    ...
    while (fscanf(file, "%s%c", word, &deliminator) != EOF) {
        ...
        if(!found) {
            // Search word does not exist in index of terms.
            if(strcmp((char *) peakPriorityQueue(terms), word) < 0) {
                term = nextPriorityQueue(terms);
                if(emptyPriorityQueue(terms)) break;
            }
            if(!strcmp((char *) peakPriorityQueue(terms), word)) found = 1;
            else while(deliminator != '\n') fscanf(file, "%s%c", word, &deliminator);
            continue;
        }
		
        if(deliminator == '\n') {
			addHashMap(words, term, newInteger(count));
			word = malloc(sizeof(char) * MAX_WORD_SIZE);
			count = 1;
